...
def is_report_safe(report: list[int], tolerance: int = 0) -> bool:
    ...
    for k, current_value in enumerate(report[1:]):
        ...
        if (change := (current_value - latest_value) * direction) > 3 or change <= 0:
            return (
                is_report_safe(
                    report[: k + 1] + report[k + 2 :], tolerance=tolerance - 1
                )
                or is_report_safe(report[:k] + report[k + 1 :], tolerance=tolerance - 1)
                or is_report_safe(report[1:], tolerance=tolerance - 1)
            )
